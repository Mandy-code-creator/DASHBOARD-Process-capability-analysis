...
if uploaded_file:
    try:
        ...
        if time_cols_for_year:
            ...
            valid_years = df['Year'].dropna().astype(int).astype(str).unique().tolist()
            year_options = sorted(valid_years, reverse=True) 
            df['Year'] = df['Year'].fillna(-1).astype(int).astype(str) 
        else:
            df['Year'] = 'N/A'
            year_options = ['N/A']
        ...
        with col_f4:
            ...
            selected_widths = st.multiselect("Order Width", options=unique_widths, default=unique_widths)
        ...
        # List to store data for the Master Table
        master_summary_data = []
